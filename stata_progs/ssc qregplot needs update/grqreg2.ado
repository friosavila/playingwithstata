	...
	*** Does your system has it???
	...
	if "`from'"!="" {
	    ** idea.. if we can do from memory, we save time and money!!
		...
		est restore `from'
		if "`e(cmd)'"=="qregplot" {
			qrgraph `varlist',  `cons'  `e(ols)' raopt(`raopt') lnopt(`lnopt') grcopt(`grcopt')  twopt(`twopt') ///
							matrixlist(e(qq) e(bs) e(ll) e(ul)) matrixols(e(bso) e(llo) e(ulo)) xlist(`e(xlist)') `label' labelopt(`labelopt')
		}		
