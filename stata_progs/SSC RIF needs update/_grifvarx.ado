...
** ucs mcs
capture program drop _grifvarx
program define _grifvarx, sortpreserve
	version 8
	...
	* uses default mean
	...
	if "`var'"!="" { 
		sort `touse' `by'
		rif_var `typlist' `varlist' = `exp' , weight(`weight') by(`by') touse(`touse')
	}
  set seed `sysseed'
 end
...
program rif_var 
	** First need to sort data and get weighted mean by group.
	syntax newvarname =/exp , [weight(str) ] touse(varname)	BY(varlist)
