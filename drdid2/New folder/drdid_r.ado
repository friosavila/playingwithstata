...
program define drdid_r, eclass byable(onecall)
        version 14
        if _by() {
...
** Move this into VCE_parse
	...
	if ("`semethod'"=="wildboot") {
	    local wboot "wboot"
		local reps = r(reps)
		local bwtype = r(bwtype)
		local cluster "`r(cluster)'"
		if ("`cluster'"!="") {
		    local wncl = r(wncl)
		}
	display in red "cluster is `cluster'"
	display in red "number of clusters is `wncl'"
	}
...
**# Verifies REPS is a number
	...
	**# Verify treatment and prepost
	...
	quietly bysort `touse' `trt' `tmt' :gen byte `vals3' = (_n == 1) * `touse'
...
program _Parse_Wildboot, rclass 
	...
	if ("`wboot1'"=="") {
		...
		else if ("`bwtype'"=="mammen") {
		    local bwtypen = 1
		}
		else if ("`bwtype'"=="rademacher") {
		    local bwtypen = 2
		}
		else if ("`bwtype'"!="rademacher" & "`bwtype'"!="mammen") {
		    display as error "invalid {bf:bwtype()}"
			di as txt "{p 4 4 2}"                           
			di as smcl as err ///
			"{bf:bwtype()} should be one of {bf:mammen} or " ///
			"{bf:rademacher}."      
			di as smcl as err  "{p_end}"
			exit 198 
		}
		return local bwtype = `bwtypen' 
		if ("`cluster'"!="") {
	    ...
	    *display "Estimating IPW logit"
		qui {		
			...
			** _delta
			...
			local ci = `level'/100
			mata:make_tbl("`att'","`cluster' ", "`touse2'", "`b'","`V'","`ciband' ","`ncl'","`wboot' ", `reps', `bwtype', `ci')
		...
		*ereturn display
		...
		qui {
			...
			*gen double `stub'att=.
			...
			local ci = `level'/100
			local touse2 `touse'
			noisily:mata:make_tbl("`att'","`cluster' ", "`touse2'", "`b'","`V'","`ciband' ","`ncl'","`wboot' ", `reps', `bwtype', `ci')			
			matrix colname `b' = ATET:r1vs0.`treatvar'
			matrix colname `V' = ATET:r1vs0.`treatvar'
			matrix rowname `V' = ATET:r1vs0.`treatvar'
			quietly count if `touse'
...
///mboot("`att'", "`touse'", "`V'","`ciband'","`cluster'", `reps', `bwtype', `ci')
