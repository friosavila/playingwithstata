...
{phang}
{bf:jwdid} {hline 2} DID estimator using Mundlack approach
...
{synopt:{opt method(method name)}}Request using other methods for the estimation of ATT's. For example {cmd:poisson}. Default is linear model.{p_end}
...
{pstd}
{cmd:jwdid} is a command/program that implements the Extended TWFE estimator proposed by Jeff Wooldridge for the estimation of ATT's based on a generalized DID designs. 

{pstd}In principle, this estimator simply suggests that to avoid some of the negative aspects of the traditional TWFE-DID estimator, which uses already treated units as controls causing the so-called negative weights, one should simple use a fully interacted set of dummies allowing for treatment effect heterogenty by cohort and timing. 
...
{pstd}This command also allows you to add controls to the model, but those controls should be time fixed. The command will NOT verify that this is the case. 

{pstd}One should also be aware that {cmd: jwdid} will create as many interactions as cohorts and periods are there in the data, times all possible controls. This could provide an extreamly large number of estimated coefficients, which could make the models difficult to estimate.

{pstd}When using other estimation methods using {cmd: method()}, the program will assume group/cohort fixed effects only, instead of individual fixed effects. Mostly, this is to avoid the incidence parameter problem.

{pstd}In the default option, only the interactions of cohort and time AFTER treatment occurred are included in the specification. This effectively uses not-yet treated as controls. When using the option {cmd:never}, all posible interactions of cohort and time are used, thus considering only the never treated as controls. Following Callaway and Sant'Anna (2021) and {cmd: csdid} 

{marker remarks}{...}
{title:Remarks}

{pstd}
The first version of this command came out as one of my attempts of learning some of the new DID methods. For a while, most of my efforts were centered on {cmd:drdid} and {cmd:csdid}. However, the methodology proposed by Prof. Wooldridge has mportant advantages over other methods, which called my applied econometrician attention. 
...
{pstd}Also, this program was coded independently from Jeff Wooldridge, but mostly based on some of his simulation codes he shared with the community. Thus all errors of interpretation, and re-implementation (some of my own views on it) are my own. If you see an error or bug, Please let me know.

{pstd}To estimate aggregates, see {help jwdid_post}
...
DID using not-treated as controls, based on group (rather than individual) fixed effects, and one time invariant control. Poisson approach
...
{pstd}This command was put together just for fun, and as my last push of "product" before my 
baby girl was born!. {p_end}

{title:Also see}

{p 7 14 2}
Help:  {help drdid}, {help csdid}, {help jwdid postestimation}, {help xtdidregress} {p_end}
